...
PetscErrorCode GetListofEdges_Power(DM networkdm,PFDATA *pfdata,PetscInt *edgelist)
{
  PetscErrorCode ierr;
  PetscInt       i,fbus,tbus,nbranches=pfdata->nbranch;
  EDGE_Power     branch=pfdata->branch;
  PetscBool      netview=PETSC_FALSE;
  PetscInt       numVertices = pfdata->nbus;
  PetscLayout     vLayout;
  ...
  ierr = PetscLayoutCreate(PetscObjectComm((PetscObject) networkdm), &vLayout);CHKERRQ(ierr);
  ...
  for (i=0; i<nbranches; i++) {
    ...
    if (rank!=0)
    {
    edgelist[2*i]   = fbus+vLayout->rstart-1;
    edgelist[2*i+1] = tbus+vLayout->rstart-1;
    }
  else
  {
  edgelist[2*i]   = fbus+vLayout->rstart;
  edgelist[2*i+1] = tbus+vLayout->rstart;
  }
    if (netview) {
